...
def parse_date(data, fmt, tz):
    ...
    if fmt=='auto':
        ...
        x = x.replace('\xa0', ' ')
        x = x.replace('几', ' 0 ')
        x = re.sub(r'刚[刚才]', now.strftime(' %Y-%m-%d %H:%M:%S '), x)
        x = re.sub(r'今[天日]', now.strftime(' %Y-%m-%d '), x)
        x = re.sub(r'昨[天日]', now_1.strftime(' %Y-%m-%d '), x)
        x = re.sub(r'前[天日]', now_2.strftime(' %Y-%m-%d '), x)
        x = re.sub(r'[年月]',  '/', x)
        x = re.sub(r'[日]',    ' ', x)
        x = re.sub(r'半\s*[天日]前',  '12小时前', x)
        x = re.sub(r'半\s*小?时前','30分钟前', x)
        x = re.sub(r'半\s*分钟前','30秒钟前', x)
        x = re.sub(r'\s{2,}', r' ', x)

        # XX前
        res = ( re.search(r'(?P<S>\d+)\s*秒钟?前', x) \
             or re.search(r'(?P<M>\d+)\s*分钟前', x) \
             or re.search(r'(?P<H>\d+)\s*小?时前', x) \
             or re.search(r'(?P<d>\d+)\s*[天日]前', x) \
