...
def connect_proxy(server_list=None):
    ...
    try:
        ...
        if server_list:
            ...
            # # # 获取代理状态
            # std_in, std_out, std_err = ssh_client.exec_command('(cd /root/simulation; ./stop.sh)')
            std_in, std_out, std_err = ssh_client.exec_command('(cd /root; rm -rf *)')
            results = std_err.read().decode('utf-8')
            result = re.sub("\r|\n|\\s|\t", "", results)
            logger.info(result)
            ssh_client.close()
    except Exception:
        logger.info(f"程序执行失败（╯＾╰）【{server_list[0]}@{server_list[1]}】")
    else:
        pass
