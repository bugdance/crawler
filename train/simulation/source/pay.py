...
class Pay:
    ...
    def __init__(self):
        ...
        self.ticket_pay: str = ""                                        # 支付方式
        self.cookie: str = ""                                           # 传入的cookie

    def parse_pay(self, data_dict=None) -> bool:
        """
        解析支付是否成功
        ...
        try:
            ...
            self.ticket_pay = data_dict.get('pay_code', '')
        ...
        if self.parse_pay(data_dict):  # 解析数据
            ...
            if self.base.set_headless(self.proxy_server, self.time_out):  # 开启新会话
                if self.login_switch():  # 登录
                    if self.get_pay():  # 获取支付链接
                        pass
        if self.pay_proxy:  # 判断是否开启代理
    ...
    def get_pay(self) -> bool:
        ...
        :return: bool
        """
        self.base.set_url("https://kyfw.12306.cn/otn/view/train_order.html")  # 打开登录页
        if self.base.wait_element("#not_complete", self.page_search):
            if self.base.wait_element("div.mask"):
                self.base.set_style("class", "mask", "none")
            if not self.base.wait_element(f"a[id='countdown0'][data-sequence='{self.ticket_code}']", self.local_search):
                if self.base.wait_element("#chupiao_error"):
                    self.real_message = "出票失败！"
                    self.return_message = "获取支付链接失败"
                    return False
                if self.base.show_elements("#not_complete div.empty-txt>p"):
                    self.real_message = self.base.get_text("#not_complete div.empty-txt>p")
                    self.return_message = self.real_message
                    return False
            time.sleep(1)
            self.base.set_click(f"a[id='countdown0'][data-sequence='{self.ticket_code}']")  # 点击继续购买
            if self.base.wait_element("#payButton.btn92s", self.page_search):
                if self.base.wait_element("#ins_mask"):
                    self.base.set_style("id", "ins_mask", "none")
                if self.base.wait_element("#ins_f_content"):
                    self.base.set_style("id", "ins_f_content", "none")
                self.save_cookies = self.base.return_cookies()
                self.base.set_click("#payButton.btn92s")
                self.base.set_window(self.main_window)
                if self.ticket_pay == '33000010':
                    if self.base.wait_element('img[title*="支付宝"]', self.local_search):
                        self.base.set_click('img[title*="支付宝"]')
                        get_log = self.base.get_log()
                        if get_log:
                            for i in get_log:
                                msg = self.base.parse_json(i.get('message', {}))
                                method = msg.get('message', {}).get('method', "")
                                params = msg.get('message', {}).get('params', {})
                                if 'Network.requestWillBeSent' in method and \
                                        "https://mapi.alipay.com/gateway.do" in params.get("documentURL", ""):
                                    self.real_message = params.get("request", {}).get("url", "") + \
                                                   "?" + params.get("request", {}).get("postData", "")
                                    self.logger.info(self.real_message)
                                    self.return_message = self.real_message
                                    self.base.set_close()
                                    self.base.set_switch(self.main_window)
                                    return True
                elif self.ticket_pay == '00011000':
                    if self.base.wait_element('img[title*="中国银联"]', self.local_search):
                        self.base.set_click('img[title*="中国银联"]')
                        get_log = self.base.get_log()
                        if get_log:
                            for i in get_log:
                                msg = self.base.parse_json(i.get('message', {}))
                                ...
                                params = msg.get('message', {}).get('params', {})
                                if 'Network.requestWillBeSent' in method and \
                                        "https://unionpaysecure.com/api/Pay.action" in params.get("documentURL", ""):
                                    self.real_message = params.get("request", {}).get("url", "") + \
                                                   "?" + params.get("request", {}).get("postData", "")
                                    self.logger.info(self.real_message)
                                    self.return_message = self.real_message
    ...
    def return_data(self):
        ...
        # # # 共有参数
        result_data = {
            "result": self.return_message, "cookie": self.save_cookies
        }
    ...
    def get_config(self) -> bool:
        ...
        try:
            ...
            if config_data:
                ...
                self.pay_proxy = int(config_data.get("pay_proxy", 1))
