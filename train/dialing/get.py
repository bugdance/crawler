...
handler = logging.FileHandler("get.log")
...
def read_excel(n) -> bool:
    ...
    """
    try:
        with xlrd.open_workbook(r'proxy.xlsx') as x:              # 打开excel
            table = x.sheets()[0]
...
def remove_queue() -> bool:
    ...
    """
    try:
        redis_queue.delete(config.PROXY_CHECK)
    except Exception as ex:
...
def connect_proxy(server_list=None) -> None:
    """
    连接远程vps服务器并上线队列
    :return: None
    """
    try:
        ...
        if server_list:
            ...
            result = re.sub("\r|\n|\\s|\t", "", results)
            # # # 在redis服务器库里建立代理服务器数据
            data = {"proxy_server": "", "proxy_user": server_list[2], "proxy_password": server_list[3], "proxy_auth": "yunku:123"}
            redis_machine.hmset(f"{server_list[0]}@{server_list[1]}", data)
            # # # 根据状态判断，如果没有状态则重新拨号
            if "running" not in result:
                logger.info(f"匹配状态失败(*>﹏<*)【{server_list[0]}@{server_list[1]}】")
                redis_queue.lpush(config.PROXY_DIALING, f"{server_list[0]}@{server_list[1]}")                       # 如果失败放到拨号队列
            else:
                re_ip = re.search(r"pppinet\d+.\d+.\d+.\d+", result)
                if not re_ip:
                    logger.info(f"代理匹配失败(*>﹏<*)【{server_list[0]}@{server_list[1]}】【{result}】")
                    redis_queue.lpush(config.PROXY_DIALING, f"{server_list[0]}@{server_list[1]}")                   # 如果失败放到拨号队列
                else:
                    ...
                    if "100%packetloss" in result:
                        logger.info(f"代理请求不通(*>﹏<*)【{server_list[0]}@{server_list[1]}】【{result}】")
                        redis_queue.lpush(config.PROXY_DIALING, f"{server_list[0]}@{server_list[1]}")               # 如果失败放到拨号队列
                    else:
                        ip = re_ip[0].replace("pppinet", "")
                        data = {"proxy_server": f"http://{ip}:3138", "proxy_user": server_list[2],
                                "proxy_password": server_list[3], "proxy_auth": "yunku:123"}
                        redis_machine.hmset(f"{server_list[0]}@{server_list[1]}", data)
                        redis_queue.lpush(config.PROXY_CHECK, f"{server_list[0]}@{server_list[1]}")                 # 如果成功放到检查队列
                        logger.info(f"添加全新代理(*^__^*)【{server_list[0]}@{server_list[1]}】")
            ssh_client.exec_command("echo 3 > /proc/sys/vm/drop_caches")
            ssh_client.close()
    except Exception as ex:
        logger.info(f"程序执行失败(*>﹏<*)【{server_list[0]}@{server_list[1]}】【{ex}】")
    else:
        pass


if __name__ == '__main__':
    num = config.GET_THREAD                                                             # 获取线程数
    if remove_queue():
