...
class McnResult(models.Model):
    ...
    active_choices = ((0, '失败'), (1, '成功'))
    active = models.SmallIntegerField(default=0, verbose_name="是否运行成功", choices=active_choices, )
    ...
    active_status.short_description = '运行结果'
        ...
        if self.every == 1:
            for period, _readable_period in SINGULAR_PERIODS:
                if period == self.period:
                    ...
                    break
            return _('间隔 {}').format(readable_period)
        for period, _readable_period in PERIOD_CHOICES:
            if period == self.period:
                readable_period = _readable_period.lower()
                break
        return _('间隔 {} {}').format(self.every, readable_period)
